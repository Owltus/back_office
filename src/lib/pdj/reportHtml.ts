/*
 * Parties PURES du rendu e-mail du petit-déjeuner, côté CLIENT (envoi manuel).
 *
 * ⚠ COPIE CONFORME de `supabase/functions/_shared/pdj/render.ts` (version Deno,
 * envoi automatique). Seuls diffèrent les imports (alias `#/` ici, relatifs côté
 * Deno). Toute modif du rendu doit être répercutée à l'identique dans les DEUX
 * fichiers — comme le couple reportHtml.ts (repjour) client / Deno.
 *
 * Même facture que `repjour/reportHtml.ts` (document responsive + cartes en
 * `<table>`) : texte d'accompagnement + les 6 tuiles de stats en HTML. Le détail
 * par chambre voyage en pièce jointe (le PDF de `pdf.ts`).
 */

import { DAY_NAMES, MONTHS } from '#/lib/shared/dates.ts'
import type { PdjSheetData, PdjStats } from '#/lib/pdj/pdf.ts'
...
/** Les 6 tuiles de stats en `<table>` (2 rangées de 3, puis 2 colonnes en mobile
 * via la media query du document). */
 ...
 * `repjour/reportHtml.ts`. Texte d'accompagnement + 6 tuiles ; PDF en pièce jointe. */
