...
import { CalendarDays, ChevronDown, Clock, Printer } from 'lucide-react'
...
export function AffichageBoard() {
  ...
  const autoSizes = useMemo(() => {
    ...
    return {
      fontSizeTitle: sizes.title,
      fontSizeMessage: sizes.message,
      fontSizeInfo: sizes.info,
  ...
  function handlePrint() {
    ...
    document.title = `Affiche_${stamp}`
    const restore = () => {
      document.title = previousTitle
    }
    // `afterprint` est plus fiable que le timeout seul (impression asynchrone) ;
    // le timeout reste en filet de sécurité, restaurer deux fois est sans effet.
