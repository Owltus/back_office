...
- **D5 — Suppression vs remboursement (mineur, acté par analogie avec `caisse_sheets`, révisé après retour d'usage).** Trois actions dans le menu contextuel : **« Rembourser »** (statut `refunded` + horodatage, réservé aux rôles `ecriture`+, cesse de compter immédiatement — D3), **« Remettre en cours »** (annule un remboursement saisi par erreur, proposée uniquement le jour même du remboursement — sert à revoir/corriger ce qui a été fait dans la journée) et **« Supprimer »**. Révision : la suppression a d'abord été réservée `gestion` seule, jugée trop stricte à l'usage — un rôle `ecriture` doit pouvoir corriger IMMÉDIATEMENT une erreur de saisie. Retenu : **suppression autorisée à l'écriture UNIQUEMENT le jour même de la prise** (`taken_date = current_date`, RLS `supabase/caisse_cautions_delete_ecriture_same_day.sql`) ; au-delà (une caution qui « court » déjà sur plusieurs jours/feuilles), seule la gestion supprime. Toutes verrouillées par `canEditFields` (D-nouveau, voir ci-dessous).
- **D9 — Verrou sur caisse clôturée (structurant, ajouté après retour d'usage).** Ajouter/modifier/rembourser/supprimer/remettre en cours une caution est verrouillé par `canEditFields` — la MÊME règle que le reste de la page (une caisse clôturée est figée pour tous, gestion comprise, jusqu'à réouverture). Avant ce correctif, ces actions restaient possibles même en consultant un shift déjà clôturé, jugé « étrange » par l'utilisateur.
