...
export function buildRepjourPdf(
  data: RepjourPdfData,
  title: string,
): jsPDF {
...
/** Rendu du document PDF en octets bruts (Uint8Array) — pour un envoi côté serveur
 * (pièce jointe Resend, upload Storage) sans passer par un data-URI. */
export function buildRepjourPdfBytes(
  data: RepjourPdfData,
  title: string,
): Uint8Array {
  const pdf = buildRepjourPdf(data, title)
