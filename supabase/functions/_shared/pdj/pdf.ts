/*
 * Génération du document PDF de la feuille de petit-déjeuner (DENO / Edge
 * Function). Rendu VECTORIEL via jsPDF (import STATIQUE npm).
 *
 * OBJECTIF : reproduire FIDÈLEMENT la trame d'impression HISTORIQUE de la page PDJ
 * (src/styles/pdj.css, bloc @media print) — en-tête « Breakfast » + date, 6 étages
 * en grille 3×2 (liseré gris en haut, sans titre d'étage), cases à cocher, et le
 * PIED de page : 5 tuiles de totaux (« PDJ non inclus » masquée à l'impression) +
 * la rangée des 3 cases « € ». MÊME document que le bouton Imprimer.
 *
 * ⚠ La fonction de dessin `renderPdjDocument` (et TOUS ses helpers de module) est
 * une COPIE CONFORME de `src/lib/pdj/pdf.ts` (version client). Seuls diffèrent
 * l'import de jsPDF (statique ici, dynamique côté client) et les entrées/sorties
 * (buildPdjPdfBytes ici, printPdjSheet côté client). Toute modification du dessin
 * doit être répercutée à l'identique dans les DEUX fichiers.
 */

import { jsPDF } from 'npm:jspdf@4.2.1'

import { floorsOf, stayKind } from './rooms.ts'

// --- Contrat de données (consommé par le code appelant) --------------------
...
/** Construit le document PDF (jsPDF) de la feuille, sans l'écrire. */
export function buildPdjPdf(data: PdjSheetData): jsPDF {
...
/** Rendu du document en octets bruts (Uint8Array) — pièce jointe Resend. */
export function buildPdjPdfBytes(data: PdjSheetData): Uint8Array {
  const pdf = buildPdjPdf(data)
